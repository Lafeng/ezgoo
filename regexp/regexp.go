...
// return.arg2: the counter of replacing loop
func (re *Regexp) ReplaceAll2(src, repl []byte) ([]byte, int) {
	n := 2
	if bytes.IndexByte(repl, '$') >= 0 {
		n = 2 * (re.numSubexp + 1)
	}
...
func (re *Regexp) ReplaceOnce(src, repl []byte) []byte {
	ma := re.FindSubmatchIndex(src)
	if len(ma) > 0 { // found
		dst := make([]byte, 0, len(src))
		if sPos := ma[0]; sPos > 0 {
			dst = append(dst, src[:sPos]...)
		}
		dst = re.Expand(dst, repl, src, ma)
		if ePos := ma[1]; ePos < len(src) {
			dst = append(dst, src[ePos:]...)
		}
		return dst
	} else { // not found
		return src
